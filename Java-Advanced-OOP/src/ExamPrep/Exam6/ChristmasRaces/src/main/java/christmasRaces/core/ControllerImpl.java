...
import christmasRaces.core.interfaces.cars.BaseCar;
import christmasRaces.core.interfaces.cars.Car;
import christmasRaces.core.interfaces.cars.MuscleCar;
import christmasRaces.core.interfaces.cars.SportsCar;
...
Race race = raceRepository.getByName(raceName);
        Map<Double,Driver> pointsPerDriver = new TreeMap<>();
if (race== null){
    throw new IllegalArgumentException(String.format(RACE_NOT_FOUND,raceName));
}
...
drivers.forEach(d-> pointsPerDriver.put(d.getCar().calculateRacePoints(laps),d));
raceRepository.remove(race);
StringBuilder sb = new StringBuilder();
        List<Driver> winners = pointsPerDriver.values().stream().limit(3).collect(Collectors.toList());
