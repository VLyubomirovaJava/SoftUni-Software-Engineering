...
public class FoodFinder {
    public static void main(String[] args) {
        ...
        while (!consonants.isEmpty()) {
            char vowel = vowels.poll(); //taking first one
            char consonant = consonants.pop(); // get last one
            ...
            for (int i = 0; i < words.length; i++) {
                ...
                if (indexVowel >= 0) {
                    foundWords[i] = foundWords[i].substring(0, indexVowel)
                            + vowel + foundWords[i].substring(indexVowel + 1);
                }
                if (indexConsonant >= 0) {
                    foundWords[i] = foundWords[i].substring(0, indexConsonant)
                            + consonant + foundWords[i].substring(indexConsonant + 1);
                }
            }
            vowels.offer(vowel);
        }
